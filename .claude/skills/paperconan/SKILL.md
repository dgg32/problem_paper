...
**Project note:** in this repo the CLI is installed into the project's `uv`-managed
`.venv`, not globally. If a bare `paperconan` command is not found, invoke it as
`uv run paperconan ...` (run from the repo root) or `.venv/bin/paperconan ...`
