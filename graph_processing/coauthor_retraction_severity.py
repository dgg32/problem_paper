...
Idempotent: recomputes both Paper properties from scratch every run.
...
def main() -> None:
    ...
    with driver.session(database=conn["database"]) as s:
        print("[coauthor_retraction_severity] resetting mid_any_count/mid_misconduct_count...", file=sys.stderr)
        s.run("MATCH (p:Paper) SET p.mid_any_count = 0, p.mid_misconduct_count = 0").consume()
